...
democracy <- read_html("https://en.wikipedia.org/w/index.php?title=The_Economist_Democracy_Index&oldid=1157018749")
...
kurto_dis <- tapply(list_by_country$X2022,list_by_country$Region,FUN = function(x) kurtosis(x) - 3)  
...
* The kurtosis is also very high for Western Europe, indicative of long tails behavior compared to the Normal distribution.
...
for (i in 1:4) {
  ...
  for (j in 1:4) {
    ...
    prob <- sum(matrix_regimes$regime_types_2006 == reg2006 & matrix_regimes$regime_types_2022 == reg2022) / sum(matrix_regimes$regime_types_2006 == reg2006)  
...
population <- read_html("https://en.wikipedia.org/w/index.php?title=List_of_countries_and_dependencies_by_population&oldid=1158466182")
...
gdp <- read_html("https://en.wikipedia.org/w/index.php?title=List_of_countries_by_GDP_(PPP)_per_capita&oldid=1156826767")
htmlgdpElement <- gdp %>% html_nodes(xpath ="/html/body/div[2]/div/div[3]/main/div[3]/div[3]/div[1]/table[1]")
list_gdp <- html_table(htmlgdpElement,fill=TRUE)

# Manual adjusts

list_gdp <- list_gdp[[1]][-1,]
...
incar <- read_html("https://en.wikipedia.org/w/index.php?title=List_of_countries_by_incarceration_rate&oldid=1153706416")
htmlincarElement <- incar %>% 
  html_nodes(xpath = '/html/body/div[2]/div/div[3]/main/div[3]/div[3]/div[1]/table[2]')

list_incar <- html_table(htmlincarElement,fill=TRUE)
list_incar <- list_incar[[1]][-1,]
...
area <- read_html("https://en.wikipedia.org/w/index.php?title=List_of_countries_and_dependencies_by_area&oldid=1157625444")
...
Based on the regression analysis, it suggests a weak or no correlation between the democracy index of a country in 2022 and its incarceration rate per 100,000 people. The estimated slope coefficient of -1.227 suggests that, on average, there is a slight decrease of 1.227 in the incarceration rate for every unit increase in the democracy index.


However, it's important to note that the high standard error of the slope coefficient (3.844) indicates a considerable degree of uncertainty in predicting the exact relationship between the democracy index and incarceration rate. The wide spread of data points around the regression line reflects the variability in the relationship.


The p-value of 0.75 suggests that we fail to reject the null hypothesis, indicating that there is insufficient evidence to support a significant correlation between the country's democracy index and the incarceration rate.

Furthermore, the intercept of 161.368 represents the predicted incarceration rate per 100,000 people in the absence of any democracy index.
...
Then, I calculated the weighted empirical CDF weighting countries by their population.


```{r}
# Manual Adjusts

list_by_country$`CIA Estimate` <-as.numeric(list_by_country$`CIA Estimate`)
...
list_by_country$PopulationPercent <- as.numeric(gsub("%", "", list_by_country$`Percentage Population`))

list_by_country2 <- list_by_country %>% 
  filter(Country != "World")
list_by_country2 <- drop_na(list_by_country2,c(`CIA Estimate`,PopulationPercent))

weights_pop <- list_by_country2 %>%
  filter(Country != "World") %>%
  filter(!is.na(PopulationPercent) & !is.na(`CIA Estimate`)) %>%
  dplyr::select(PopulationPercent)


pop <- ewcdf(list_by_country2 %>%
               filter(!is.na(PopulationPercent) & !is.na(`CIA Estimate`)) %>%
               dplyr::select(`CIA Estimate`) %>%
               unlist(), 
             weights = weights_pop$PopulationPercent)


plot(pop, xlab="GDP (PPP)", ylab = "CDF Random Person", main="CDF of World Population GDP (PPP) Per Capita")
quantiles_y <- quantile(pop, c(0, 0.25, 0.5, 0.75, 1))
...
Similarly, I calculated the weighted empirical CDF weighting countries by their land area
...
list_by_country3 <- list_by_country %>% 
  filter(Country != "World")
list_by_country3 <- drop_na(list_by_country3,c(`CIA Estimate`,land_area))

area_w <- ewcdf(list_by_country3 %>% filter(!is.na(land_area)&!is.na(`CIA Estimate`)) %>% select(`CIA Estimate`) %>% unlist(), 
               weights = list_by_country3 %>% filter(!is.na(land_area)&!is.na(`CIA Estimate`)) %>% select(land_area) %>% unlist())

plot(area_w, xlab="GDP (PPP)", ylab = "CDF Random Person", main="CDF of World Population GDP (PPP) Per Capita")
quantiles_z <- quantile(area_w, c(0, 0.25, 0.5, 0.75, 1))
...
The median value for Z is higher than the median value for X, indicating a higher GDP per capita when considering individuals selected uniformly at random from all land areas on Earth. (Median of Y $>$ Median of X $>$ Median of Z)

In summary, Z demonstrates higher GDP per capita values compared to the other random variables, as evidenced by its higher medians and higher percentiles. This suggests that the GDP per capita distributions for individuals selected from land areas tend to be higher overall than the distribution for countries and individuals. These differences can be attributed to the sampling criteria and variations in economic development, population distribution, and geographic factors.
